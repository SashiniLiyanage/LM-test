...
class UpdateLibrary extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      libFilename : this.props.data.libFilename,
      libType : this.props.data.libType,
      libLicenseURL: "",
      comment: ""
    };
  }
  ...
  handleURLChange = (event) => {
      const libLicenseURL = event.target.value;
      this.setState({ libLicenseURL });
  }
  
  submitHandler = (e) => {
    e.preventDefault()
    if (this.state.libLicenseID.length === 0) {
      alert("Please add the licenses!!")
    } else {
        this.updateLibrary()
    }
  }
  updateLibrary = () => {
    axios.post(process.env.REACT_APP_BE_URL + '/LicenseManager/addLibraryRequest/'+this.state.packName, this.state,
    {
      headers:{
      ...
      .then(response => {
        ...
        this.setState({libLicenseID:null})
          ...
          >
            <Grid container spacing={3}>
              ...
              <Grid item xs={12}>
                <TextValidator
                  required
                  label="URL to support selected license"
                  value={this.state.libLicenseURL}
              ...
              <Grid item xs={12}>
                <Button type="submit" color="primary" variant="contained">Request LIcenses</Button>
              </Grid>
            </Grid>
          </ValidatorForm>
        </React.Fragment>
